...
export function checkChanges(root, options = {}) {
  ...
  const config = loadConfig(snapshot.root, options.scope);
  if (options.scope) snapshot.files = snapshot.files.filter(f => f.path.startsWith(`${options.scope}/`) || f.oldPath.startsWith(`${options.scope}/`));
  ...
  return { schemaVersion: 1, mode: config.mode, owners: config.owners, requiredChecks: config.requiredChecks, packageScope: options.scope ?? null, scope: snapshot.staged ? 'staged' : options.base ? `changes since ${options.base} merge base` : 'working tree including untracked files', head: snapshot.head,
...
export function renderCheck(report) {
  ...
  for (const f of report.findings) lines.push(`[${f.baselined ? 'baseline' : f.severity}] ${f.rule} ${JSON.stringify(f.path)}: ${f.title}\n  ${f.next}`);
