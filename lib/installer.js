import { existsSync, readFileSync, readdirSync, lstatSync, mkdirSync, writeFileSync, unlinkSync } from 'node:fs';
import { resolve, join, dirname, relative, isAbsolute } from 'node:path';
import { createHash } from 'node:crypto';

export const adapters = {
  cursor: ['.cursorrules', '.cursor'],
  copilot: ['.github/copilot-instructions.md', '.github/instructions/governance-security.instructions.md'],
  claude: ['CLAUDE.md'],
  aider: ['.aider'],
  devin: ['AGENTS.md'],
};
...
export function planInstall(source, target, tools = []) {
  ...
  const selected = [...new Set([...state.tools.filter(tool => Object.hasOwn(adapters, tool)), ...tools])];
  ...
  const changes = files.map(name => {
    ...
    const managed = adapterFiles.has(name) && !name.startsWith('.cursor/') && !name.startsWith('.github/instructions/');
...
export function planUninstall(target) {
  target = resolve(target);
  if (!existsSync(safePath(target, statePath))) throw new Error('No installation manifest');
  const state = loadState(target);
  const changes = Object.entries(state.files).map(([path, record]) => {
    const dest = safePath(target, path);
    if (!existsSync(dest)) return { path, status: 'missing' };
    const current = read(dest);
    const tracked = record.managed ? blockContent(current) : current;
    if (tracked === null || hash(tracked) !== record.hash) return { path, status: 'preserve' };
    const content = record.managed ? current.replace(tracked, '').trim() : '';
    return { path, status: content ? 'remove-block' : 'remove', content: content ? `${content}\n` : '' };
  });
  return { target, changes };
}
export function applyUninstall(plan) {
  for (const item of plan.changes) {
    const dest = safePath(plan.target, item.path);
    if (item.status === 'remove') unlinkSync(dest);
    if (item.status === 'remove-block') writeFileSync(dest, item.content);
  }
  // Keep the manifest while conflicts exist so a later removal can be reviewed.
  if (!plan.changes.some(item => item.status === 'preserve')) unlinkSync(safePath(plan.target, statePath));
}
