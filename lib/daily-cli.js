...
import { startPilot, stopPilot, pilotEvent, summarizePilot } from './pilot.js';
import { readLocal } from './local-state.js';
...
export async function runDaily(command, args) {
  if (['pilot-start', 'pilot-stop', 'pilot-feedback', 'pilot-report', 'pilot-help'].includes(command)) {
    const opts = parseOptions(args, ['--project', '--tool', '--rule', '--rating'], []);
    const root = repository(resolve(opts['--project'] ?? '.'));
    if (command === 'pilot-start') console.log(JSON.stringify(startPilot(root, opts['--tool']), null, 2));
    else if (command === 'pilot-stop') { stopPilot(root); console.log('Local measurement disabled. Existing records remain in Git metadata.'); }
    else if (command === 'pilot-report') console.log(JSON.stringify(summarizePilot(readLocal(root, 'pilot.json')), null, 2));
    else if (command === 'pilot-help') { pilotEvent(root, 'setup_help'); console.log('Setup assistance recorded locally if pilot is enabled.'); }
    else { pilotEvent(root, 'feedback', { rule: opts['--rule'], rating: opts['--rating'] }); console.log('Feedback recorded locally if pilot is enabled.'); }
    return true;
  }
  ...
  if (command === 'check' || command === 'baseline') {
    ...
    if (command === 'baseline') {
      ...
      pilotEvent(root, 'check', { durationMs: Date.now() - started, findings: report.findings.length, missingTools: report.coverage.missingTools.length });
    }
    return true;
  }
  ...
  if (command === 'init') {
    ...
    if (!opts['--dry-run']) {
      ...
      pilotEvent(root, 'init', { durationMs: Date.now() - started });
    }
