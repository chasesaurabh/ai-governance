...
const frameworkVersion = JSON.parse(readFileSync(new URL('../package.json', import.meta.url), 'utf8')).version;
const recordName = (name, scope) => scope ? `${name}-${digest(scope).slice(0, 16)}.json` : `${name}.json`;
const trustDigest = (root, config) => digest({ root, config });
export function trustProject(root, scope = '') {
  root = repository(root); const config = loadConfig(root, scope);
  const trust = { schemaVersion: 1, digest: trustDigest(root, config), confirmedAt: new Date().toISOString() };
  writeLocal(root, recordName('trust', scope), trust); return trust;
}
...
export async function runChecks(root, { selected, timeoutMs = 120000, signal, force = false, scope = '' } = {}) {
  root = repository(root); const config = loadConfig(root, scope);
  if (!Number.isInteger(timeoutMs) || timeoutMs < 1 || timeoutMs > 3600000) throw new Error('Timeout must be 1–3600000 ms');
  if (readLocal(root, recordName('trust', scope))?.digest !== trustDigest(root, config)) throw new Error('Review project commands/code, then run trust --accept. Config changes require renewed trust.');
  ...
  const previous = readLocal(root, recordName('runs', scope), {});
  const results = [];
  for (const key of [...new Set(keys)]) {
    ...
    const observed = await execute(command, scope ? safePath(root, scope) : root, timeoutMs, signal);
    const result = { ...observed, key, command, fingerprint, cached: false, observedAt: new Date().toISOString(), observer: { node: process.version, platform: process.platform, framework: frameworkVersion } };
    if (sourceFingerprint(root, config) !== fingerprint) result.status = 'stale';
    previous[key] = result; results.push(result);
    writeLocal(root, recordName('runs', scope), previous);
...
export function verificationSummary(root, scope = '') {
  root = repository(root); const config = loadConfig(root, scope), fingerprint = sourceFingerprint(root, config);
  const records = readLocal(root, recordName('runs', scope), {});
  return [...new Set([...Object.keys(config.commands).filter(k => config.commands[k]), ...config.requiredChecks])].map(key => {
