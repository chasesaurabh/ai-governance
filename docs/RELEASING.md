...
Publishing: configure npm's trusted publisher for this repository and `.github/workflows/release.yml`, and configure the `npm-release` GitHub environment with the desired reviewers. Push a reviewed tag exactly matching package.json, such as v2.0.1. The workflow verifies, tests the archive and publishes through OIDC. Do not reuse an already published version. See https://docs.npmjs.com/trusted-publishers/ for npm account setup. Repository code alone cannot configure account ownership or grant publishing rights.
